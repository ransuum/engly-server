...
public class UserController {
    ...
    @ApiResponse(responseCode = "200", description = "User found successfully.", content = @Content(schema = @Schema(implementation = UsersDto.class)))
    @ApiResponse(responseCode = "403", description = "Forbidden. User is not an administrator.", content = @Content)
    @GetMapping("/{id}")
    ...
    @ApiResponse(responseCode = "200", description = "List of users retrieved successfully.", content = @Content)
    @ApiResponse(responseCode = "403", description = "Forbidden. User is not an administrator.", content = @Content)
    @GetMapping
    @PreAuthorize("hasAuthority('SCOPE_ADMIN')")
